...
class Protocol(object):
    def __init__(self, h, name='protocol', tstop=0, dt=0.025, nruns=1):
        ...
        self.nruns = nruns
        self.totalsteps = int(round(self.tstop / self.h.dt)) + 1
    ...
    def getStepWaveform(self, t_list, v_list):
        ...
        for (t, v) in zip(t_list, v_list):
            v_tmp = np.append(v_tmp, v * np.ones(int(round(t / self.h.dt))) + 1)
    ...
    def record(self, cell):
        ...
        self.i_vec.record(cell.soma(0.5)._ref_ik)
    ...
    def updateMat(self, idx):
        self.vMat[idx][:] = np.array(self.v_vec.to_python())
        tmp = np.array(self.i_vec.to_python())
        self.iMat[idx][:] = tmp
        self.tMat[idx][:] = np.array(self.t_vec.to_python())
...
class Activation(Protocol):
    def __init__(self, h, dt=0.025):
        self.nruns = 16
        self.vsteps = np.linspace(-80.0, 70.0, self.nruns)
        super(Activation, self).__init__(h, name='act', tstop=700, dt=dt, nruns=self.nruns)

    def playWaveform(self, step):
        self.vWave = self.h.Vector(self.getStepWaveform([100.0, 500.0, 100.0], [-80.0, step, -80.0]))
        self.vc.dur1 = self.tstop
        self.vWave.play(self.vc._ref_amp1, self.h.dt)
...
class Inactivation(Protocol):
    def __init__(self, h, dt=0.025):
        ...
        super(Inactivation, self).__init__(h, name='inact', tstop=1750, dt=dt, nruns=self.nruns)
...
class Deactivation(Protocol):
    def __init__(self, h, dt=0.025):
        self.nruns = 15
        self.vsteps = np.linspace(-100.0, 40.0, self.nruns)
        super(Deactivation, self).__init__(h, name='deact', tstop=700, dt=dt, nruns=self.nruns)
...
class Ramp(Protocol):
    def __init__(self, h, dt=0.025):
        self.nruns = 1
        super(Ramp, self).__init__(h, name='ramp', tstop=2900, dt=dt, nruns=self.nruns)
...
class ActionPotential(Protocol):
    def __init__(self, h, dt=0.025):
        self.nruns = 1
        super(ActionPotential, self).__init__(h, name='ap', tstop=1800, dt=dt, nruns=self.nruns)
